...
ui_intro <- column(width = 12,
  ...
  p(
    "This tool may take a few moments to fully load.  The tool is ready when",
    "the maps are visible."
    )
  )
...
ui_q2_title <- column(width = 12,
  ...
  p(style = "max-width: 33%;",
    ...
    "  I recommend setting the threshold at 9 million."
    )
  )
...
StateColor <- function(the_map, state_col) {
  ...
  v <- seq(from = 0.5, to = 0.8, length.out = 4) %>% rep(each = 3)
...
SplitStates <- function(the_state, n, state_col, c_data = county_data) {
  ...
  k_centers <- kmeans(k_centers, centers = n, iter.max = 10^3, nstart = 10^1)
...
y3 <- reactive({
  ...
  if (input$merge_check4) {
    if (input$metro_unifier == "Yes") {
      new_states <- ReplaceState("NJ", "Phil")
...
cm <- reactive({
  county_map$map_zero <- y0()[data_to_map]
  county_map$map_one <- y1()[data_to_map]
  county_map$map_two <- y2()[data_to_map]
  county_map$map_three <- y3()[data_to_map]
  county_map$map_four <- y4()[data_to_map]
  county_map
  })
...
## color scheme
c1 <- reactive({StateColor(cm(), "map_one")})
c2 <- reactive({StateColor(cm(), "map_two")})
c3 <- reactive({StateColor(cm(), "map_three")})
c4 <- reactive({StateColor(cm(), "map_four")})
...
output$map_three <- renderPlot({
  map_base() +
    geom_polygon(
      data = cm(),
      mapping = aes(x = lon, y = lat, group = group,
        color = map_three, fill = map_three)
      ) + 
...
shinyApp(ui, server)
warning("Don't forget to program MSA table, drill out cm() issue")
